 ...
 * has no completed analysis to tailor against.
 */
export const generateDocument = (offerId: number, type: DocumentType, language: string) =>
  request<GeneratedDocument>(
    `/api/offers/${offerId}/documents${query({ type, language })}`,
    { method: 'POST' },
  )

export const getLatestDocument = (offerId: number, type: DocumentType) =>
  requestOrNull<GeneratedDocument>(
    `/api/offers/${offerId}/documents/latest${query({ type })}`,
  )
